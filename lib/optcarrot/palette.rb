...
module Optcarrot
  ...
  module Palette
    ...
    # @rbs return: Array[Array[Integer]]
    def defacto_palette
      [
        ...
        [
          ...
          r = [((rgb >> 16 & 0xff) * rf).floor, 0xff].min || raise
          g = [((rgb >>  8 & 0xff) * gf).floor, 0xff].min || raise
          b = [((rgb >>  0 & 0xff) * bf).floor, 0xff].min || raise
          [r, g, b]
        end
      end
    end
    ...
    def nestopia_palette
      (0..511).map do |n|
        ...
        levels = [[-0.12, 0.40], [0.00, 0.68], [0.31, 1.00], [0.72, 1.00]][level] || raise
        level0 = levels[0] #: Float
        level1 = levels[1] #: Float
        ...
        [[105, 0.570], [251, 0.351], [15, 1.015]].map do |angle, gain|
          ...
          [0, (clr * 255).round, 255].sort[1] || raise
        end
      end
    end
  end
